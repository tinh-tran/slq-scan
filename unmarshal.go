...
func Unmarshal(data interface{}, rows *sql.Rows) error {
	...
	if ptr.Kind() != reflect.Ptr {
		return errors.New("enou: data must be a pointer.")
	}
	if ptr.IsNil() {
		return errors.New("enou: data is a nil pointer.")
	}
	...
	if len(columns) == 0 {
		return errors.New("enou: no columns.")
	}
	target := ptr.Elem()
	switch target.Kind() {
	case reflect.Slice:
...
func scan2Struct(rows *sql.Rows, columns []columnType, target reflect.Value) error {
	...
	for _, column := range columns {
		...
		if !field.IsValid() {
			return errors.New("enou: no or multiple field '" + column.FieldName + "' in struct")
		}
